...
func handleWithDecode[T Context, S Validator[T], U interface{}](
	...
	patternWithMethod := method + " " + pattern //POST /hello

	// https://arc.net/l/quote/kdxxhrfn about zero-length K array
	...
	r.mux.HandleFunc(patternWithMethod, func(w http.ResponseWriter, req *http.Request) {
		...
		ctx := ctxPointer.Interface().(T) // *MyContext
		...
		for _, h := range handlers {
			...
			default:
				...
				if resp != (*U)(nil) {
					...
					if err != nil {
						log.Println("error writing respomse:", err)
						return
					}
				}
			}
